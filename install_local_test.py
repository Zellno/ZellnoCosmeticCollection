...
PROJECT = HOME / "dayz/modding/ZellnoCosmeticCollection"
BUILD_MOD = PROJECT / "build/@ZellnoCosmeticCollection"

SERVER = HOME / "dayz/server"
SERVER_MOD = SERVER / "@ZellnoCosmeticCollection"
SERVER_KEY = SERVER / "keys/Zellno.bikey"

CLIENT = HOME / ".local/share/Steam/steamapps/common/DayZ"
CLIENT_MOD = CLIENT / "@ZellnoCosmeticCollection"
...
MOD_NAME = "@ZellnoCosmeticCollection"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Instala Zellno Cosmetic Collection de forma reversível."
        )
    )
    parser.add_argument(
        "--apply",
    ...
    require_file(
        BUILD_MOD / "addons/ZellnoCosmeticCollection.pbo"
    )
    require_file(
        BUILD_MOD
        / "addons/ZellnoCosmeticCollection.pbo.Zellno.bisign"
    )
    ...
    print("=" * 76)
    print(" Zellno Cosmetic Collection — instalação local")
    print("=" * 76)
    print()
    ...
    backup = (
        BACKUP_ROOT
        / f"install-zellno-cosmetic-collection-{timestamp}"
    )
    ...
    if server_existed:
        if SERVER_MOD.is_symlink():
            ...
            shutil.copytree(
                SERVER_MOD,
                backup / "@ZellnoCosmeticCollection.server",
            )
        else:
            shutil.copy2(
                SERVER_MOD,
    ...
    if client_existed:
        if CLIENT_MOD.is_symlink():
            ...
            shutil.copytree(
                CLIENT_MOD,
                backup / "@ZellnoCosmeticCollection.client",
            )
        else:
            shutil.copy2(
                CLIENT_MOD,
    ...
    stage = Path(
        tempfile.mkdtemp(
            prefix=".ZellnoCosmeticCollection-stage-",
            dir=SERVER,
        )
    )

    try:
        staged_mod = stage / "@ZellnoCosmeticCollection"
        ...
        server_backup = backup / "@ZellnoCosmeticCollection.server"
        ...
        client_backup = backup / "@ZellnoCosmeticCollection.client"
