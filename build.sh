...
SOURCE_DIR="$PROJECT_DIR/addons/ZellnoCosmeticCollection"
BUILD_ROOT="$PROJECT_DIR/build"
BUILD_MOD="$BUILD_ROOT/@ZellnoCosmeticCollection"
...
EXPECTED_BUILD_MOD="$PROJECT_DIR/build/@ZellnoCosmeticCollection"
...
for required in \
    ...
    "$SOURCE_DIR/data/wojtek_tshirt_co.paa" \
    "$PROJECT_DIR/mod.cpp" \
...
wine "$FILEBANK" \
    -property prefix=ZellnoCosmeticCollection \
    -dst "$(winepath -w "$BUILD_ADDONS")" \
    "$(winepath -w "$SOURCE_DIR")"

PBO="$BUILD_ADDONS/ZellnoCosmeticCollection.pbo"
